...
func update_bottom_panel(animated_shape: AnimatedShape3D):
...
func on_inspector_edited_object_changed():
	...
	if edited_object is ShapeFrame3D:
		return
	if edited_object is CollisionShape3D and (edited_object as CollisionShape3D).owner == null:
		return  # we're editing the previews' shape
	if edited_object is Shape3D:
		return  # same
	if edited_object == null:
		return  # same
	var animated_shape: AnimatedShape3D = null
	if edited_object is AnimatedShape3D:
		animated_shape = edited_object as AnimatedShape3D
