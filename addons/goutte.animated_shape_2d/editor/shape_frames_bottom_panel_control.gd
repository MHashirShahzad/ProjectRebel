...
func rebuild_gui(
	for_animated_shape: AnimatedShape3D,
	force := false,
):
	...
	if not is_instance_valid(self.animated_shape.animated_sprite):
		var label := Label.new()
		label.text = tr("Please assign an input animated sprite to this AnimatedShape3D.")
		self.frames_container.add_child(label)
		missing_requirements = true
	if not is_instance_valid(self.animated_shape.collision_shape):
		var label := Label.new()
		label.text = tr("Please assign a target collision shape to this AnimatedShape3D.")
		self.frames_container.add_child(label)
		missing_requirements = true
	if not is_instance_valid(self.animated_shape.shape_frames):
		var label := Label.new()
		label.text = tr("Please create or load shape frames data for this AnimatedShape3D.")
		self.frames_container.add_child(label)
		missing_requirements = true
...
func rebuild_animation_names_item_list(
	...
	if animated_shape == null:
		print("AnimatedShape3D: no animated shape is configured.")
		return
	if animated_shape.animated_sprite == null:
		print("AnimatedShape3D: no animated sprite is configured.")
		return
	if animated_shape.animated_sprite.sprite_frames == null:
		print("AnimatedShape3D: no sprite frames is configured.")
		return
...
func on_change_do_reload(_property: String):
	var inspector := EditorInterface.get_inspector()
	if not (inspector.get_edited_object() is AnimatedShape3D):
		return
	if inspector.property_edited.is_connected(on_change_do_reload):
		inspector.property_edited.disconnect(on_change_do_reload)
