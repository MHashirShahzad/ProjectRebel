...
var animated_shape: AnimatedShape3D
...
func configure(
	animated_shape: AnimatedShape3D,
...
func get_shape_frame() -> ShapeFrame3D:
...
func set_shape_frame(value: ShapeFrame3D):
	if self.animated_shape == null:
		return
	if self.animated_shape.shape_frames == null:
		return
...
func update():
	...
	var collision_shape_parent_transform := Transform3D()
	if self.animated_shape.collision_shape.get_parent() is Node3D:
...
var sprite_preview: AnimatedSprite3D
var preview_background: ColorRect
var preview_shape: CollisionShape3D
...
func preview_shape_frame():
	if not is_instance_valid(sprite_preview):
		...
		sprite_preview.name = "PreviewAnimatedSprite3D"
	...
	if shape_frame != null:
		if not is_instance_valid(preview_shape):
			preview_shape = CollisionShape3D.new()
			preview_shape.name = "PreviewCollisionShape3D"
...
func _on_create_button_pressed():
	...
	if self.animated_shape.collision_shape.shape:
		...
		shape_frame.shape = BoxShape3D.new()
	self.animated_shape.shape_frames.set_shape_frame(
...
func _on_paste_button_pressed():
	...
	if copied_instance_id == null:
		return
	var pasted_shape_frame: ShapeFrame3D = instance_from_id(copied_instance_id)
	if pasted_shape_frame == null:
		return
	if not (pasted_shape_frame is ShapeFrame3D):
		return
