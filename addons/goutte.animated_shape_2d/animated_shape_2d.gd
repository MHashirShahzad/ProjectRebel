...
# 4. Load or Create a ShapeFrames3D (it's our database)
# 
# Notes:
...
@export var shape_frames: ShapeFrames3D

## If [code]true[/code], use the initial shape in the target CollisionShape2D
## as fallback when the shape is not defined in the ShapeFrames3D.
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if self.shape_frames == null:
		warnings.append("This node requires a ShapeFrames3D to store data.  Make a new one?")
	return warnings
...
func get_current_shape_frame() -> ShapeFrame3D:
...
func update_collision_shape_position(new_position: Vector3):
...
func update_collision_shape_shape(new_shape: Shape3D):
	...
	if (
		...
		if (
			(self.collision_shape.shape is BoxShape3D) #RectangleShape2D
			and
			(new_shape is BoxShape3D)
		):
			self.collision_shape.shape.size = new_shape.size
			return
		
		if (
			(self.collision_shape.shape is SphereShape3D) # Circle shape 2d
			and
			(new_shape is SphereShape3D)
		):
			self.collision_shape.shape.radius = new_shape.radius
			return
		
		if (
			(self.collision_shape.shape is CapsuleShape3D)
			and
			(new_shape is CapsuleShape3D)
		):
		...
		if (
			(self.collision_shape.shape is SeparationRayShape3D) # Segment shape 2d ray
			and
			(new_shape is SeparationRayShape3D)
		):
		...
		if (
			(self.collision_shape.shape is WorldBoundaryShape3D)
			and
			(new_shape is WorldBoundaryShape3D)
		):
...
func tween_collision_shape_shape():
	...
	if (
		(self.collision_shape.shape is BoxShape3D)
		and
		(self.target_collision_shape_shape is BoxShape3D)
	):
	...
	if (
		(self.collision_shape.shape is SphereShape3D)
		and
		(self.target_collision_shape_shape is SphereShape3D)
	):
	...
	if (
		(self.collision_shape.shape is CapsuleShape3D)
		and
		(self.target_collision_shape_shape is CapsuleShape3D)
	):
	...
	if (
		(self.collision_shape.shape is SeparationRayShape3D) # line sepratation ray
		and
		(self.target_collision_shape_shape is SeparationRayShape3D)#
	):
...
func is_collision_shape_parent_flippable() -> bool:
	return (
		...
		and
		not (self.collision_shape_parent is PhysicsBody3D)
	)
