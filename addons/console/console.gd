...
func _enter_tree() -> void:
	...
	line_edit.placeholder_text = "Enter \"ls\" to see availible commands"
...
func _ready() -> void:
	add_command(":q", quit, 0, 0, "Quits the game.")
	add_command("clear", clear, 0, 0, "Clears the text on the console.")
	add_command("del_hs", delete_history, 0, 0, "Deletes the history of previously entered commands.")
	add_command(":h", help, 0, 0, "Displays instructions on how to use the console.")
	add_command("ls", commands_list, 0, 0, "Lists all commands and their descriptions.")
...
func on_text_entered(new_text : String) -> void:
	...
	if not new_text.strip_edges().is_empty():
		...
		var text_command := text_split[0].to_lower() # Convert to lower case as all commands will be lower
...
func help() -> void:
	rich_label.append_text("	Built in commands:
		...
		[color=light_green]cmd[/color] | Shows a reduced list of all the currently registered commands
		[color=light_green]ls[/color] | Shows a detailed list of all the currently registered commands
		[color=light_green]del_hs[/color] | Deletes the commands history
		[color=light_green]:q[/color] | Quits the game
	Controls:
